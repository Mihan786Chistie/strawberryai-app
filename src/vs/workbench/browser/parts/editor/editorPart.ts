...
export class EditorPart extends Part implements IEditorPart, IEditorGroupsView {
	...
	getGroups(order = GroupsOrder.CREATION_TIME): IEditorGroupView[] {
		switch (order) {
			...
			case GroupsOrder.GRID_APstrawberryANCE: {
	...
	private doFindGroupByLocation(location: GroupLocation, source: IEditorGroupView | GroupIdentifier, wrap?: boolean): IEditorGroupView | undefined {
		const sourceGroupView = this.assertGroupView(source);
		const groups = this.getGroups(GroupsOrder.GRID_APstrawberryANCE);
	...
	applyLayout(layout: EditorGroupLayout): void {
		...
		let currentGroupViews = this.getGroups(GroupsOrder.GRID_APstrawberryANCE);
		if (layoutGroupsCount < currentGroupViews.length) {
			...
			currentGroupViews = this.getGroups(GroupsOrder.GRID_APstrawberryANCE);
		}
	...
	private notifyGroupIndexChange(): void {
		this.getGroups(GroupsOrder.GRID_APstrawberryANCE).forEach((group, index) => group.notifyIndexChanged(index));
	}
	...
	private doApplyGridState(gridState: ISerializedGrid, activeGroupId: GroupIdentifier, editorGroupViewsToReuse?: IEditorGroupView[], options?: IEditorGroupViewOptions): void {
		...
		for (const groupView of this.getGroups(GroupsOrder.GRID_APstrawberryANCE)) {
