...
app_dir=$(pwd)
target_path="$app_dir/extensions/strawberryai-submodule/extensions/vscode"
link_path="$app_dir/extensions/strawberryai-ref"

# Run the base functionality
echo -e "\nInitializing sub-modules..."

# Check if the submodule directory already exists
if [ -d "$app_dir/extensions/strawberryai-submodule" ]; then
    echo "Removing existing strawberryai-submodule directory"
    execute "rm -rf $app_dir/extensions/strawberryai-submodule" "Failed to remove existing strawberryai-submodule directory"
fi
...
execute "cd ./extensions/strawberryai-submodule" "Failed to change directory to extensions/strawberryai-submodule"
