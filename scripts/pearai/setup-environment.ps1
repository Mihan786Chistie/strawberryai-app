...
function Initialize-BaseFunctionality {
    ...
    Set-Location .\extensions\strawberryai-submodule
...
function Create-SymLink {
    Write-Host "`nCreating symbolic link 'extensions\strawberryai-ref' -> 'extensions\strawberryai-submodule\extensions\vscode'" -ForegroundColor White
...
$modulePath = Join-Path -Path $currentDir -ChildPath 'extensions\strawberryai-submodule'
$targetPath = Join-Path -Path $modulePath -ChildPath 'extensions\vscode'
$linkPath = Join-Path -Path $currentDir -ChildPath 'extensions\strawberryai-ref'
